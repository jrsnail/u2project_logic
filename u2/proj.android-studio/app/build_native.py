...
def get_num_of_cpu():
    ...
    try:
        ...
        print "Can't know cpuinfo, use default 8 cpu"
        return 8
...
def copy_files(src, dst):
    ...
    for item in os.listdir(src):
        ...
        if os.path.isdir(path):
            ...
            if not (os.path.exists(new_dst) and os.path.isdir(new_dst)):
                os.mkdir(new_dst)
...
def copy_dir(dir):
    if os.path.exists(dir) and os.path.isdir(dir):
        if len(os.listdir(dir)) == 0:
            print "dir name %s is empty dir " % dir
            return
...
def copy_file(filePath):
    if os.path.isfile(filePath):
        tempPath = filePath.replace("libs", "temp_libs")
        head, tail = os.path.split(tempPath)
        if head and tail and not os.path.exists(head):
            os.makedirs(head)
        elif os.path.exists(tempPath):
            os.remove(tempPath)
        os.renames(filePath, tempPath)
