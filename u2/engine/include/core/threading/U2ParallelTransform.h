 ...
 */
#ifndef __U2ParallelTransform_H__
#define __U2ParallelTransform_H__
...
class TransformThreadPoolImpl_Queue<InputIterator, Last, OutputIterator, Function, true>
...
transform(InputIterator first, const Last& last, OutputIterator result, Function&& fun)
{
	unsigned int tc =
    ...
    else 
    {
        ...
        >(first, last, result, fun, thread_count, maxpart != 1 ? maxpart : 3 * (tc + 1));
        return result;
	}
}
...
typename std::decay<OutputIterator>::type
transform(Container& container, OutputIterator&& result, Function&& fun)
{
	return transform<thread_count, maxpart
...
#endif // __U2ParallelTransform_H__
